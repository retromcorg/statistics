		...
		success: function (response) {
			...
			if (response.messages) {
				...
				response.messages.forEach(e => {
					...
					result += `<div class="col-md-12"><span data-toggle="tooltip" title="${moment.unix(e.timestamp).fromNow()}"><img src="https://skins.legacyminecraft.com/avatars/${e.uuid}?size=28&overlay" alt="${e.username}"></span> <a href="../player/${e.uuid}" id="display">${motdtocolor(e.display_name, e.username)}</a>: ${e.message}</div>`
...
function loadPlayerModel() {
		...
		success: function (response) {
			if (response.totalPunishments != 0) {
				...
				if (response.banned) {
					$(".banned").prop({
						"id": "is_banned",
						"title": "This user has this color because they have been banned from the server."
					});
					$(".banned").attr({
						"data-toggle": "banned",
						"data-placement": "left"
				...
				response.bans.forEach(ban => {
					...
					result += `<tr><td><img src="https://skins.legacyminecraft.com/avatars/${ban.admin[1]}?size=25&overlay"></td><td><a href="../player/${ban.admin[1]}">${ban.admin[0]}</a></td><td>${ban.reason}</td><td>${ban.pardoned}</td><td>${e.join(", ")}</td></tr>`;
				});
				result += '</tbody></table>';
	...
	if (checkWebGLSupport()) {
		let skinViewer = new skinview3d.SkinViewer({
			...
			skin: `https://skins.legacyminecraft.com/skins/${$("#q_uuid").text()}`
		});
	...
	$.ajax({
		...
		success: function (response) {
			...
			response.data.forEach((val, key) => {
				...
				builder += `<td><img src="https://skins.legacyminecraft.com/avatars/${val.uuid}?size=28&overlay">&emsp;<a href="./player/${val.uuid}">${val.username}</a></td>`;
		...
		success: function (response) {
			...
			if (response.cnt != 0) {
				...
				response.online.forEach(e => {
					...
					result += `<div class="col-md-6"><span data-toggle="tooltip" title="x: ${Math.floor(e.coords.x)}, y: ${Math.floor(e.coords.y)}, z: ${Math.floor(e.coords.z)}, world: ${e.coords.world}"><img src="https://skins.legacyminecraft.com/avatars/${e.uuid}?size=28&overlay" alt="${e[0]}"></span> <a href="player/${e.uuid}" id="display">${ motdtocolor(e.display, e.username)}</a></div>`
